...
Let's make a correlation matrix to see the relationships of variables.
...
We can see that no_of_repeated_guest is positively correlated with no_of_previous_bookings_not_canceled. Arrival year is negatively correlated with arrival_month. However, the correlation matrix indicates that there are too many predictors, some of which are more or less pointless. Therefore, we will select a few key predictors later on to simplify and clarify our model's predictions.

Additonally, let's make a histogram to see the distribution of the booking status.
...
From the plot, we can see that "Yes" has far more outcomes than "No," indicating that many people cancel their reservations. This means we will have a substantial amount of data for analysis.

Lastly, let's make a ggplot of booking_status by lead_time.
